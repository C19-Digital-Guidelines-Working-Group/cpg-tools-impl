...
@SpringBootTest
@ContextConfiguration(classes = CPGTestConfig.class)
@TestPropertySource(properties = {
    "scan.packages=edu.mayo.kmdp.kdcaci.knew.trisotech",
    "edu.mayo.kmdp.trisotechwrapper.repositoryName=CPG",
    "edu.mayo.kmdp.trisotechwrapper.repositoryId=15386d61-0e35-42ff-a69d-5f9011f0a4dc"})
public class CPGAssetDownloader {
  ...
  @Test
  @Disabled
  void testEDCPGModels() {
    File tgtFolder = getTgtFolder();

    if (!tgtFolder.exists()) {
      assertTrue(tgtFolder.mkdirs());
    }

    Answer<List<Pointer>> assets = assetRepository.listKnowledgeAssets();

    assetRepository.listKnowledgeAssets()
        .ifPresent(l -> l.forEach(assetPtr -> saveArtifacts(assetPtr, tgtFolder)));
//        .ifPresent(l -> l.forEach(assetPtr -> System.out.println(">>>>>> Model " + assetPtr.getName())));
  }
